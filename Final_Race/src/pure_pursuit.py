...
def purepursuit_control_node(data):
    ...
    # Retrieve current velocity (if available from odometry, otherwise estimate or use last command)
    ...
    lookahead_distance = 0.8 + (0.15 * current_vel)
    lookahead_distance = max(0.5, min(lookahead_distance, 2.5)) # Clamp values
    ...
    target_velocity = plan[target_index][2]
    command.speed = target_velocity # Use the pre-calculated optimal speed
    
    # steering_difficulty = abs(steering_angle_rad) 

    # if steering_difficulty < math.radians(10):
    #     command.speed = 40.0  # Max speed on straights
